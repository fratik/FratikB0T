...
public class BetaDao implements Dao<Beta> {
    private final EventBus eventBus;
    private final PgMapper<Beta> mapper;

    BetaDao(ManagerBazyDanych managerBazyDanych, EventBus eventBus) {
        if (managerBazyDanych == null) throw new IllegalStateException("managerBazyDanych == null");
        mapper = managerBazyDanych.getPgStore().mapSync(Beta.class);
        this.eventBus = eventBus;
    }

    @Override
    public Beta get(String id) {
        return mapper.load(id).orElse(new Beta(id, false));
    }
