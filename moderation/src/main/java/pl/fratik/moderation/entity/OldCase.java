...
public class OldCase {
    ...
    private static OldCasesDao casesDao;
    ...
    OldCase(@NotNull String userId, @NotNull String guildId, int caseId, @Nullable TemporalAccessor timestamp, String messageId, @NotNull Kara type) {
        this.userId = userId;
    ...
    public static int getNextCaseId(@NotNull OldCaseRow caseRow) {
        int lastId = 0;
        for (OldCase aCase : caseRow.getCases()) {
            lastId = Math.max(aCase.caseId, lastId);
        }
        return lastId + 1;
    }

    public static OldCase getCaseById(int caseId, Guild guild) {
        if (casesDao == null) throw new IllegalStateException("casesDao jest nie ustawiony");
        return getCaseById(caseId, casesDao.get(guild));
    }

    @SuppressWarnings("WeakerAccess")
    public static OldCase getCaseById(int caseId, OldCaseRow caseRow) {
        return getCaseById(caseId, caseRow.getCases());
    }

    public static OldCase getCaseById(int caseId, List<OldCase> cList) {
        for (OldCase aCase : cList) if (aCase.getCaseId() == caseId) return aCase;
        return null;
    }

    public static void setStaticVariables(OldCasesDao casesDao, ScheduleDao scheduleDao) {
        OldCase.casesDao = casesDao;
        OldCase.scheduleDao = scheduleDao;
    }
