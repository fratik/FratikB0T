...
public class OldCaseSerializer extends StdSerializer<List<OldCase>> {

    public OldCaseSerializer() {
        this(null);
    }

    public OldCaseSerializer(Class<List<OldCase>> vc) {
        super(vc);
    }

    @Override
    public void serialize(List<OldCase> caseList, JsonGenerator jsonGenerator, SerializerProvider serializerProvider) throws IOException {
        List<ParsedCase> parsedCaseList = new ArrayList<>();
        for (OldCase aCase : caseList) {
            ...
            pCase.setFlagi(OldCase.Flaga.getRaw(aCase.getFlagi()));
            if (aCase.getDowody() != null && !aCase.getDowody().isEmpty()) {
                pCase.setDowody(aCase.getDowody().stream().map(d -> new ParsedDowod(d.getId(),
                        Long.toUnsignedString(d.getAttachedBy()), d.getContent())).collect(Collectors.toList()));
            }
