...
public class PlaylistCommand extends MusicCommand {
    ...
    public void save(NewCommandContext context) {
        ...
        if (queueDao.get(kolejka.getId()) != null) {
            save(context);
            return;
        }
        kolejka.setPiosenki(mms.getKolejka());
        queueDao.save(kolejka);
        context.reply(context.getTranslated("playlist.saved", kolejka.getId()));
    }
    ...
    public void load(NewCommandContext context) {
        ...
        if (q == null || q.getPiosenki().isEmpty()) {
            context.reply(context.getTranslated("playlist.load.notfound"));
            return;
        }
        ...
        context.reply(context.getTranslated("playlist.loaded"));
