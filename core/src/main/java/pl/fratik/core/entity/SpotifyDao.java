...
public class SpotifyDao implements Dao<SpotifyConfig> {
    ...
    public SpotifyDao(ManagerBazyDanych managerBazyDanych, EventBus eventBus) {
        if (managerBazyDanych == null) throw new IllegalStateException("managerBazyDanych == null");
        mapper = managerBazyDanych.getPgStore().mapSync(SpotifyConfig.class);
    ...
    public SpotifyConfig get(Guild guild) {
        return get(guild.getId());
    }

    @Override
    public List<SpotifyConfig> getAll() {
        return mapper.loadAll();
    }

    @Override
    public void save(SpotifyConfig toCos) {
